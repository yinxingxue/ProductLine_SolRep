 ...
 */
public class JinghuiCrossover extends Crossover {
	/**
  ...
  private static final List VALID_TYPES = Arrays.asList(RealSolutionType.class,
	 ...
	 * Constructor
	 */
	public JinghuiCrossover(HashMap<String, Object> parameters) {
		super(parameters) ;
	...
	public Object execute(Object object) throws JMException {
		...
		int numberOfVariables = parent[0].getDecisionVariables().length;
		jrand = PseudoRandom.randInt(0, numberOfVariables - 1);
		CR_ = PseudoRandom.randDouble(0, 1);		
		
		F_ = PseudoRandom.randDouble(0, 1);
		double F2 = PseudoRandom.randDouble(0, 1);
		for (int j=0; j < numberOfVariables; j++) {
			if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
				int x0 = (int) current.getDecisionVariables()[j].getValue();
				int x1 = (int) parent[0].getDecisionVariables()[j].getValue();
				int x2 = (int) parent[1].getDecisionVariables()[j].getValue();
				int x3 = (int) parent[2].getDecisionVariables()[j].getValue();
				int x4 = (int) parent[3].getDecisionVariables()[j].getValue();		
				int x5 = (int) parent[4].getDecisionVariables()[j].getValue();		
				...
				double phi = v1 * F_ + v2 * F2 - v1 * F_ * v2 * F2;
				...
				int value = (int) current.getDecisionVariables()[j].getValue();
				child.getDecisionVariables()[j].setValue(value);
			} // else
		}
		return child ;
