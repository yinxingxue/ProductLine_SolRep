...
// ver CÃ¡lculo de estructuras VIII.35
 ...
 public double BucklingOmega(double Nxx, double[] G, double[] B) throws JMException{
	 ...
	 double _ip = 1.0; // coeficiente de Pandeo
  ...
  if(Nxx < 0.0 && G[AREA] > 0.0 && lBuckling)
  {
      ...
      double _io  = B[L_] * G[BLijY_] / iy;
      // esbeltez maxima entre para los ejes y, z
      _io = Math.min(_io, B[L_] * G[BLijZ_] / iz);
	      ...
	      else if(G[TypeMaterial_]== 2)  // F-24 steel 24 MN/m2
	      {
	    	  ...
	    	  // double _ii = -2.3443 + 1.07817 * _io - 0.0036181 * Math.pow(_io, 2) + 0.000008209862 * Math.pow(_io, 3) - 0.00000001058458 * Math.pow(_io, 4) + 0.000000000007147864 * Math.pow(_io, 5) - 0.00000000000000196410755 * Math.pow(_io, 6);
	          // coeficiente omega de mayoración por efecto de Pandeo
	          // double ratio = _io / _ii;
	          // _ip = Math.pow(ratio, 2.0);
              
	    	  // Buckling coefficient, Table: Lamda-Omega
              double _ii  = _io;
              if ( _ii <= 150)
              {
            	  _ip = 1.113 + 0.0070516 * _ii - 0.000132108 * Math.pow(_ii, 2.0) + 0.000002106132 * Math.pow(_ii, 3.0) - 0.00000000397368332151 * Math.pow(_ii, 4.0);
              }
              else _ip = 25.0;
	      }
          ...
          else if(G[TypeMaterial_] == 20)  // Wood hard an halt-hard
	      {
             ...
             if (_ii<=150)
             {
                 _ip = 1.048 + 0.005524 * _ii - 0.000101666 * Math.pow(_ii, 2.0) + 0.00000301687 * Math.pow(_ii, 3.0) - 0.000000004366246 * Math.pow(_ii, 4.0);
             }
             else _ip = 25.0;
	      }
      }
